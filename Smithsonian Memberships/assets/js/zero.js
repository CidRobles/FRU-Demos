 ...
 */
var arrayOnce = [80, 175, 300, 600, 1500, 3500]
var arrayMonthly = [80, 175, 300, 600, 1500, 3500]
var defaultOnce = 600
var premiumMinOnce = 75
var defaultMonthly = 600
...
var campaignContent = {
    hero: {
        url: './assets/img/si-memberships.jpg',
        alt: 'Make Your Impact with  a  Gifted Membership'
    },
    logo: {
        url: './assets/img/si-logo.png',
        alt: 'Smithsonian'
    },
    title: 'Make Your Impact with  a  Gifted Membership',
    text: "As a Friend of the Smithsonian, you'll provide a reliable source of income that brings the entire Institution to life — and you'll enjoy a suite of exclusive benefits and privileges available to members only. Every membership includes a subscription to the award-winning Smithsonian magazine, and so much more."
}
...
function renderStepZero(frequency) {
    ...
    this.document.getElementById('FRU-step-zero-donate').addEventListener('click', function () {
        if (document.getElementById('gift-fname-zero').value == '') {
            renderToast('The first name of the giftee is required')
            document.getElementById('gift-fname-zero').focus()
            return
        } else {
            zeroState.form.gifteeFirstName = document.getElementById('gift-fname-zero').value
        }

        if (document.getElementById('gift-lname-zero').value == '') {
            renderToast('The last name of the giftee is required')
            document.getElementById('gift-lname-zero').focus()
            return
        } else {
            zeroState.form.gifteeLastName = document.getElementById('gift-lname-zero').value
        }

        if (document.getElementById('gift-email-zero').value == '') {
            renderToast('The email address of the giftee is required')
            document.getElementById('gift-email-zero').focus()
            return
        } else {
            zeroState.form.gifteeEmail = document.getElementById('gift-email-zero').value
        }

        if (document.getElementById('gift-address-zero').value == '') {
            renderToast('The address line of the giftee is required')
            document.getElementById('gift-address-zero').focus()
            return
        } else {
            zeroState.form.gifteeAddress = document.getElementById('gift-address-zero').value
        }

        if (document.getElementById('gift-city-zero').value == '') {
            renderToast('The city of the giftee is required')
            document.getElementById('gift-city-zero').focus()
            return
        } else {
            zeroState.form.gifteeCity = document.getElementById('gift-city-zero').value
        }

        if (document.getElementById('gift-state-zero').value == '') {
            renderToast('Please provide the state of the giftee')
            return
        } else {
            zeroState.form.gifteeState = document.getElementById('gift-state-zero').value
        }

        if (document.getElementById('gift-zip-zero').value == '') {
            renderToast('The ZIP code of the giftee is required')
            document.getElementById('gift-zip-zero').focus()
            return
        } else {
            zeroState.form.gifteeZip = document.getElementById('gift-zip-zero').value
        }

        FundraiseUp.openCheckout('FUNELKXPEBW', {
            donation: {
                amount: 600,
                currency: 'USD',
                recurring: 'monthly'
